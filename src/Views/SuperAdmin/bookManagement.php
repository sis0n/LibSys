...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const resultsIndicator = document.getElementById("resultsIndicator"); // Ito na 'yung <h4>
    
    const paginationControls = document.getElementById("paginationControls");
    const paginationList = document.getElementById("paginationList");

    if (!bookTableBody || !addBookModal || !editBookModal || !importModal || !searchInput || !paginationList || !resultsIndicator) {
    ...
    } catch (e) { console.error("SessionStorage Error:", e); currentPage = 1; }
    ...
    addBookModal?.addEventListener("click", e => { if (e.target === addBookModal) closeModal(addBookModal); });
    ...
    editBookModal?.addEventListener("click", e => { if (e.target === editBookModal) closeModal(editBookModal); });
    ...
    input?.addEventListener('change', (e) => {
        ...
        if (!allowedTypes.includes(file.type)) {
            ...
            input.value = ''; uploadText.textContent = 'Upload Image';
            previewContainer.classList.add('hidden'); previewImage.src = ''; return;
        }
    ...
    editInput?.addEventListener('change', (e) => {
        ...
        if (!allowedTypes.includes(file.type)) {
            alert('Invalid file type! Please upload only JPG or PNG files.');
            editInput.value = ''; editUploadText.textContent = 'Upload Image';
            editPreviewContainer.classList.add('hidden'); editPreviewImage.src = ''; return;
        }
    ...
    function setupDropdown(btnId, menuId) {
        const btn = document.getElementById(btnId); const menu = document.getElementById(menuId); if (!btn || !menu) return;
        const closeAllDropdowns = () => { document.querySelectorAll('.absolute.mt-1.z-20').forEach(m => m.classList.add('hidden')); }
        btn.addEventListener("click", e => { e.stopPropagation(); const isHidden = menu.classList.contains('hidden'); closeAllDropdowns(); if(isHidden) menu.classList.toggle("hidden"); });
    }
    setupDropdown("statusDropdownBtn", "statusDropdownMenu");
    setupDropdown("sortDropdownBtn", "sortDropdownMenu");
    document.addEventListener("click", () => { document.querySelectorAll('.absolute.mt-1.z-20').forEach(menu => menu.classList.add('hidden')); });
    
    window.selectSort = (el, val) => {
        const valueEl = document.getElementById("sortDropdownValue"); if(valueEl) valueEl.textContent = el.textContent;
        document.querySelectorAll("#sortDropdownMenu .sort-item").forEach(i => i.classList.remove("bg-orange-50", "font-semibold")); if(el) el.classList.add("bg-orange-50", "font-semibold");
        currentSort = val; currentPage = 1; 
        try { sessionStorage.removeItem('bookManagementPage'); } catch(e){} 
        loadBooks(currentPage); 
    };
    window.selectStatus = (el, val) => {
        const valueEl = document.getElementById("statusDropdownValue"); if(valueEl) valueEl.textContent = val;
        document.querySelectorAll("#statusDropdownMenu .status-item").forEach(i => i.classList.remove("bg-orange-50", "font-semibold")); if(el) el.classList.add("bg-orange-50", "font-semibold");
        currentStatus = val; currentPage = 1; 
        try { sessionStorage.removeItem('bookManagementPage'); } catch(e){} 
        loadBooks(currentPage); 
    };
    
    const defaultSort = document.querySelector("#sortDropdownMenu .sort-item"); if(defaultSort) defaultSort.classList.add("bg-orange-50", "font-semibold");
    const defaultStatus = document.querySelector("#statusDropdownMenu .status-item"); if(defaultStatus) defaultStatus.classList.add("bg-orange-50", "font-semibold");
    ...
    async function loadBooks(page = 1) {
        ...
        try {
            ...
            const res = await fetch(`/libsys/public/superadmin/books/fetch?${params.toString()}`);
            ...
            if (data.success && Array.isArray(data.books)) {
                ...
                try { sessionStorage.setItem('bookManagementPage', currentPage); } catch (e) {} 
    ...
    paginationList.addEventListener('click', (e) => {
        e.preventDefault(); if (isLoading) return; const target = e.target.closest('a[data-page]'); if (!target) return;
        const pageStr = target.dataset.page; if (pageStr === '...') return;
    ...
    addBookForm?.addEventListener("submit", async (e) => {
         e.preventDefault(); const formData = new FormData(addBookForm);
         if (!formData.get('accession_number') || !formData.get('call_number') || !formData.get('title') || !formData.get('author')) { Swal.fire('Missing Info', 'Please fill in all required fields (*).', 'warning'); return; }
         try {
             const res = await fetch("/libsys/public/superadmin/books/store", { method: "POST", body: formData });
             const result = await res.json();
             if (result.success) { Swal.fire('Success!', result.message || 'Book added!', 'success'); closeModal(addBookModal); addBookForm.reset(); previewContainer.classList.add('hidden'); uploadText.textContent = 'Upload Image'; loadBooks(1); }
             else { Swal.fire('Error', result.message || 'Failed to add book.', 'error'); }
         } catch (err) { console.error("Add book error:", err); Swal.fire('Error', 'An error occurred.', 'error'); }
    });

    window.editBook = async (bookId) => {
        if (!bookId) return; currentEditingBookId = bookId;
        try {
            const res = await fetch(`/libsys/public/superadmin/books/get/${bookId}`);
            ...
            if (data.success && data.book) {
                ...
                editUploadText.textContent = 'Change Image'; editInput.value = '';
                if(book.cover) { editPreviewImage.src = book.cover; editPreviewContainer.classList.remove('hidden'); }
                else { editPreviewContainer.classList.add('hidden'); editPreviewImage.src = ''; }
                openModal(editBookModal);
            } else { Swal.fire('Error', data.message || 'Could not find book details.', 'error'); }
        } catch (err) { console.error("Edit book fetch error:", err); Swal.fire('Error', 'Error fetching book data.', 'error'); }
    };

    editBookForm?.addEventListener("submit", async (e) => {
        e.preventDefault(); if (!currentEditingBookId) return;
        const formData = new FormData(editBookForm);
         if (!formData.get('accession_number') || !formData.get('call_number') || !formData.get('title') || !formData.get('author')) { Swal.fire('Missing Info', 'Please fill in all required fields (*).', 'warning'); return; }
         try {
             const res = await fetch(`/libsys/public/superadmin/books/update/${currentEditingBookId}`, { method: "POST", body: formData });
             const result = await res.json();
             if (result.success) { Swal.fire('Success!', result.message || 'Book updated!', 'success'); closeModal(editBookModal); loadBooks(currentPage); }
             else { Swal.fire('Error', result.message || 'Failed to update book.', 'error'); }
         } catch (err) { console.error("Update book error:", err); Swal.fire('Error', 'An error occurred.', 'error'); }
    });
    
    window.deleteBook = async (bookId, title) => {
         if (!bookId) return;
         const result = await Swal.fire({ title: 'Are you sure?', text: `Delete "${title}"?`, icon: 'warning', showCancelButton: true, confirmButtonColor: '#d33', cancelButtonColor: '#6b7280', confirmButtonText: 'Yes, delete it!' });
        if (result.isConfirmed) {
            try {
                const res = await fetch(`/libsys/public/superadmin/books/delete/${bookId}`, { method: "POST" });
                const result = await res.json();
                if (result.success) { Swal.fire('Deleted!', result.message || 'Book deleted.', 'success'); loadBooks(currentPage); }
                else { Swal.fire('Error', result.message || 'Failed to delete.', 'error'); }
            } catch (err) { console.error("Delete book error:", err); Swal.fire('Error', 'An error occurred.', 'error'); }
        }
    };
