...
const showFinalModal = (isSuccess, title, message) => {
    ...
    const theme = isSuccess ? {
        ...
        iconBg: 'bg-green-100', iconColor: 'text-green-600', iconClass: 'ph-check-circle',
        progressBarColor: 'bg-green-500',
    } : {
        bg: 'bg-red-50', border: 'border-red-300', text: 'text-red-700',
        iconBg: 'bg-red-100', iconColor: 'text-red-600', iconClass: 'ph-x-circle',
        progressBarColor: 'bg-red-500',
    };

    Swal.fire({
        showConfirmButton: false, showCancelButton: false, buttonsStyling: false,
        width: '450px', backdrop: `rgba(0,0,0,0.3) backdrop-filter: blur(6px)`, timer: duration, 
        ...
        `,
        customClass: { popup: "!block !bg-transparent !shadow-none !p-0 !border-0 !w-auto !min-w-0 !max-w-none" },
    });
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadCollegeOptions(currentCollegeId = null) {
        ...
        try {
            ...
            departmentSelect.disabled = true;
            showProfileToast('ph-x-circle', 'Error', 'Failed to load department list.', 'error', 3000);
        }
    }
    ...
    profileForm?.addEventListener('submit', async (e) => {
        ...
        if (missingFields.length) {
            showProfileToast('ph-warning', 'Missing Info', `Please fill in all required fields. (Missing: ${missingFields.join(', ')})`, 'warning');
            return;
        }
        ...
        showLoadingModal("Saving profile...", "Updating your details. This might take a moment.");
    ...
    saveCrop?.addEventListener('click', () => {
        ...
        circleCanvas.toBlob(blob => { 
            croppedBlob = blob; 
            // 🟢 SUCCESS TOAST
            showProfileToast('ph-check-circle', 'Image Cropped', 'Picture is ready to be saved with the profile changes.', 'success', 3000);
