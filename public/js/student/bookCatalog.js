window.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadCart() {
    try {
      const r = await fetch(`${BASE_URL}/student/cart/json`);
      if (!r.ok) throw Error();
      cart = await r.json();
      updateCartBadge();
    } catch (e) {
  ...
  async function addToCart(id) {
    if (!id) return;
    try {
      const r = await fetch(`${BASE_URL}/student/cart/add/${id}`);
  ...
  async function removeFromCart(id) {
    if (!id) return;
    try {
      const r = await fetch(`${BASE_URL}/student/cart/remove/${id}`, {
        method: "POST"
      });
  ...
  async function performClearCart() {
    try {
      const r = await fetch(`${BASE_URL}/student/cart/clear`, {
        method: "POST"
      });
  ...
  async function loadBooks(page = 1) {
    ...
    try {
      ...
      const res = await fetch(`${BASE_URL}/student/bookCatalog/fetch?${params.toString()}`);
  ...
  async function loadAvailableCount() {
    try {
      const r = await fetch(`${BASE_URL}/student/bookCatalog/availableCount`);
      if (!r.ok) throw Error();
      const d = await r.json();
